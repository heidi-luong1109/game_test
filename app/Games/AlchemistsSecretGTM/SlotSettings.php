...
namespace VanguardLTE\Games\AlchemistsSecretGTM
{
    class SlotSettings
    {
        ...
        public function GetBonusPreset($bet, $lines)
        {
            ...
            global $dblink;
            $bank = $this->Bank / $bet;
            $_obf_0D3D11063F0B3C1E1D365C1E122104310B3219022B3D11 = \VanguardLTE\BonusPreset::whereRaw('win <= ? and game = ? and game_lines = ?', [
                $bank, 
                $this->slotId, 
                $lines
            ])->get();
            $presets = [];
            foreach( $_obf_0D3D11063F0B3C1E1D365C1E122104310B3219022B3D11 as $a => $b ) 
            {
                $presets[] = ['info' => $b->info];
            }
            if( count($presets) > 0 ) 
            {
                $_obf_0D2E32130C39312B3D2C1F0C10040B35151C1724312832 = array_rand($presets);
                return $presets[$_obf_0D2E32130C39312B3D2C1F0C10040B35151C1724312832];
            }
            else
            {
                return 'NONE';
        ...
        public function GetHistory()
        {
            ...
            foreach( $history as $log ) 
            {
                ...
                $_obf_0D0B0D3817302B3E125C5B1617241E25402C1624403201 = json_decode($log->str);
                if( $_obf_0D1C182417322707301D3D0523132F1D092E3316302332->responseEvent != 'gambleResult' ) 
                {
                    $_obf_0D0B0D3817302B3E125C5B1617241E25402C1624403201->serverResponse->FreeSeries = '';
                    $this->lastEvent = json_encode($_obf_0D0B0D3817302B3E125C5B1617241E25402C1624403201);
                    break;
                }
            }
        ...
        public function UpdateJackpots($bet, $isJack)
        {
            ...
            for( $jp = 1; $jp <= 1; $jp++ ) 
            {
                if( $this->jpgs[$jp - 1]->balance != '' ) 
                {
                    ...
                    if( $this->jpgs[$jp - 1]->pay_sum <= $this->slotJackpot[$jp - 1] && !$isJackPay && $this->jpgs[$jp - 1]->pay_sum > 0 && $isJack ) 
                    {
                        $isJackPay = 1;
                        $isJackId = $jp - 1;
